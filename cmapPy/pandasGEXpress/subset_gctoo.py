...
def subset_gctoo(gctoo, row_bool=None, col_bool=None, rid=None, cid=None,
    ...
    Args:
        gctoo (GCToo object)
        row_bool (list of bools): length must equal gctoo.meth_df.shape[0]
        col_bool (list of bools): length must equal gctoo.meth_df.shape[1]
    ...
    rows_to_keep_bools = gctoo.meth_df.index.isin(rows_to_keep)
    cols_to_keep_bools = gctoo.meth_df.columns.isin(cols_to_keep)

    # Make the output gct
    out_gctoo = GCToo.GCToo(
        src=gctoo.src, version=gctoo.version,
        meth_df=gctoo.meth_df.loc[rows_to_keep_bools, cols_to_keep_bools],
        cov_df=gctoo.cov_df.loc[rows_to_keep_bools, cols_to_keep_bools],
        row_metadata_df=gctoo.row_metadata_df.loc[rows_to_keep_bools, :],
        col_metadata_df=gctoo.col_metadata_df.loc[cols_to_keep_bools, :])

    assert out_gctoo.meth_df.size > 0, "Subsetting yielded an empty gct!"

    logger.info(("Initial GCToo with {} rows and {} columns subsetted down to " +
                 "{} rows and {} columns.").format(
                      gctoo.meth_df.shape[0], gctoo.meth_df.shape[1],
                      out_gctoo.meth_df.shape[0], out_gctoo.meth_df.shape[1]))
...
def get_rows_to_keep(gctoo, rid=None, row_bool=None, ridx=None, exclude_rid=None):
    ...
    if rid is not None:
        ...
        rows_to_keep = [gctoo_row for gctoo_row in gctoo.meth_df.index if gctoo_row in rid]
    ...
    # Use row_bool if provided
        ...
        assert len(row_bool) == gctoo.meth_df.shape[0], (
            "row_bool must have length equal to gctoo.meth_df.shape[0]. " +
            "len(row_bool): {}, gctoo.meth_df.shape[0]: {}".format(
                len(row_bool), gctoo.meth_df.shape[0]))
        rows_to_keep = gctoo.meth_df.index[row_bool].values
    ...
    # Use ridx if provided
        ...
        assert max(ridx) <= gctoo.meth_df.shape[0], (
            "ridx contains an integer larger than the number of rows in " +
            "the GCToo. max(ridx): {}, gctoo.meth_df.shape[0]: {}").format(
                max(ridx), gctoo.meth_df.shape[0])

        rows_to_keep = gctoo.meth_df.index[ridx].values

    # If rid, row_bool, and ridx are all None, return all rows
    else:
        rows_to_keep = gctoo.meth_df.index.values
